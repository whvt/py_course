...
def created_booking(base_url):
    return create_booking(base_url, booking_data)
...
def test_update_booking(base_url, auth_token, created_booking):
    ...
    response = update_booking(base_url, created_booking, auth_token, updated_data)
    assert response.status_code == 200
...
def test_delete_booking(base_url, auth_token, created_booking):
    response = delete_booking(base_url, created_booking, auth_token)
...
def test_create_booking_missing_fields(base_url):
    ...
    assert response.status_code in [400, 500]  # API should reject incomplete data
